...
type PaymentStore interface {
	ByJob(ctx context.Context, firebaseUID, jobID string) (domain.Payment, error)
	MarkPaidOffline(ctx context.Context, firebaseUID, jobID string) (domain.Payment, error)
}
...
func (s *PostgresPaymentStore) ByJob(ctx context.Context, firebaseUID, jobID string) (domain.Payment, error) {
	const query = `
		...
		WHERE p.job_id = $1 AND hirer.firebase_uid = $2`
	payment, err := scanPayment(s.db.QueryRow(ctx, query, jobID, firebaseUID))
...
func (s *PostgresPaymentStore) MarkPaidOffline(ctx context.Context, firebaseUID, jobID string) (domain.Payment, error) {
	const query = `
